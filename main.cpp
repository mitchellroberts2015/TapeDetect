...
static void findSquares( const Mat& image, vector<vector<Point> >& squares, double min, double max )
{
    squares.clear();
            ...
            for( size_t i = 0; i < contours.size(); i++ )
            {
                    ...
                    isContourConvex(Mat(approx)) )
                {
                    ...
                    if( maxCosine < 0.3 )
                    {
                        ...
                        double ratio = abs((double)bound.height / bound.width);
...
int goalIsHot()
{
    ...
    findSquares(image, squares, 3, 8);
    findSquares(image, squares2, 1.0/8, 1.0/3);
    
    if (squares.size()>0 && squares2.size()>0)
    ...
    drawSquares(image, squares, Scalar(0,255,255));
    drawSquares(image, squares2, Scalar(0,255,0));

	imshow("frc", image);
	waitKey(1);

    return squares.size()>0 && squares2.size()>0;
}
